...
class Stage {
  ...
  setCamera(y: number, speed: number = 0.3) {
    TweenLite.to(
        this.camera.position, speed, {y: y + 4, ease: Power1.easeInOut});
...
class Block {
  ...
  constructor(block: Block) {
    ...
    let geometry = new THREE.BoxGeometry(
        this.dimension.width, this.dimension.height, this.dimension.depth);
  ...
  place(): BlockReturn {
    ...
    let blocksToReturn:
        BlockReturn = {plane: this.workingPlane, direction: this.direction};
    ...
    if (overlap > 0) {
      ...
      let choppedGeometry = new THREE.BoxGeometry(
          choppedDimensions.width, choppedDimensions.height,
          choppedDimensions.depth);
      choppedGeometry.applyMatrix(new THREE.Matrix4().makeTranslation(
          choppedDimensions.width / 2, choppedDimensions.height / 2,
      ...
      choppedMesh.position.set(
          choppedPosition.x, choppedPosition.y, choppedPosition.z);
